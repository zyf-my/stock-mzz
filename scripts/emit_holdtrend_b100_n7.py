...
def run(cur,mask,p1,p2,n,beta):
    ...
    return out
cfg=load_config('configs/hist_lgbm_alpha_x.yaml'); d=load_panel(str(resolve_data_path(cfg,None))); drop_other_label(d,'y1'); v,t=slice_split(d,'valid'),slice_split(d,'test'); y=np.asarray(d['y1'],np.float32); cv=np.load(OUT/'fusion_alpha_platform_mix_valid.npy'); ct=np.load(OUT/'fusion_alpha_platform_mix_test.npy'); p1v,p2v=y[v['start']-1],y[v['start']-2]; p1t,p2t=y[t['start']-1],y[t['start']-2]; pv=run(cv,v['mask_x'],p1v,p2v,7,1.0); pt=run(ct,t['mask_x'],p1t,p2t,7,1.0); tag='task1_fusion_holdtrend_b100_n07'; np.save(OUT/f'{tag}_valid.npy',pv); np.save(OUT/f'{tag}_test.npy',pt); save_submission(pt,ROOT/f'submissions/{tag}.npy'); print(tag,float(np.nanmean(rank_ic_series(pv,v['y1'],v['mask_y']))))
