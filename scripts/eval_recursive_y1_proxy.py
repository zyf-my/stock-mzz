"""Evaluate a causal recursive y1-history proxy with no test labels."""
...
def rank_day(x, mask):
    """Rank finite values only; missing stocks get zero and are handled by fallback."""
    x = np.asarray(x, dtype=np.float32)
    mask = np.asarray(mask, dtype=bool) & np.isfinite(x)
    return panel_cs_rank(np.nan_to_num(x, nan=0.0)[None, :], mask[None, :])[0]


def recursive_predictions(current, mask, initial_prev, weight, update_mode="mixed"):
    ...
    for t in range(current.shape[0]):
        ...
        out[t] = cur_r
        out[t, use_prev] = ((1.0 - weight) * cur_r[use_prev] + weight * prev_r[use_prev]).astype(np.float32)
        # Feed only model-derived information into the next day. No y1 is used.
        if update_mode == "current":
            prev = cur_r
        else:
            prev = out[t]
...
def main() -> None:
    ...
    # Last known labeled day before each split. The test split uses valid's last label
    # only as initialization; every later test day is fully recursive.
    prev_v = np.asarray(data["y1"][valid["start"] - 1], dtype=np.float32)
    prev_t = np.asarray(data["y1"][test["start"] - 1], dtype=np.float32)
    rows = []
    candidates = []
    for mode in ["mixed", "current"]:
      for w in [0.0, .05, .1, .15, .2, .25, .3, .4, .5, .55, .6, .65, .7, .75, .8, .85, .9, .95]:
        pv = recursive_predictions(cur_v, valid["mask_x"], prev_v, w, mode)
        ic = mean_rank_ic(pv, valid["y1"], valid["mask_y"])
        s = rank_ic_series(pv, valid["y1"], valid["mask_y"])
        row = (float(ic), float(np.nanmean(s[-60:])), float(w), mode)
        rows.append(row)
        candidates.append((row, pv))
    rows.sort(reverse=True, key=lambda x: x[0])
    print("BEST", rows)
    (OUT / "recursive_y1_proxy_grid.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
    best = max(candidates, key=lambda x: x[0][0])
    if best[0][0] > 0.12799:
        w = best[0][2]
        mode = best[0][3]
        pt = recursive_predictions(cur_t, test["mask_x"], prev_t, w, mode)
        tag = f"recursive_{mode}_w{int(w * 100):02d}"
        ...
        (OUT / f"fusion_alpha_{tag}_lock.json").write_text(
            json.dumps({"valid_ic": best[0][0], "last60_ic": best[0][1], "weight": w, "update_mode": mode, "causal": True}, indent=2),
            encoding="utf-8",
        )
