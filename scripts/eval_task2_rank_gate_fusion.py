"""Task2 fusion: same locked recipe as x6_today, but the coverage gate is RANK.

y2 tree fusion already emits ranks (~1..4700). Raw-blending them with GRU (~0.5)
is a no-op. This is the one-variable fix: space raw -> rank.

Does not overwrite submissions/task2_fusion_x6_today.npy.
"""
...
def main() -> None:
    ...
    x6_v, x6_t = _load("gru_x6_with_today_valid.npy"), _load("gru_x6_with_today_test.npy")
    o6_v, o6_t = _load("gru_only6_with_today_valid.npy"), _load("gru_only6_with_today_test.npy")
    n6_v, n6_t = _load("gru_next6_with_today_valid.npy"), _load("gru_next6_with_today_test.npy")
    tree_v, tree_t = _load("fusion_valid.npy"), _load("fusion_test.npy")
    mlp_v, mlp_t = _load("cs_mlp_valid.npy"), _load("cs_mlp_test.npy")
    mlp6_v, mlp6_t = _load("cs_mlp_only6_valid.npy"), _load("cs_mlp_only6_test.npy")
    ...
    print(f"rank coverage gate RankIC={mean_rank_ic(gated_v, y, my):.6f}")
    ...
    print(f"task2 rank-gate fusion RankIC={ic:.6f}")

    OUT.mkdir(parents=True, exist_ok=True)
    np.save(OUT / "fusion_rank_gate_valid.npy", out_v)
    np.save(OUT / "fusion_rank_gate_test.npy", out_t)
    summary = {
        "label_key": LABEL_KEY,
        "gru_ens_ic": float(mean_rank_ic(ens_v, y, my)),
        "rank_gate_ic": float(mean_rank_ic(gated_v, y, my)),
        "fusion_valid_ic": float(ic),
        "recipe": "task1 x6_today weights; coverage gate space=rank (tree already ranked)",
    }
    (OUT / "fusion_rank_gate_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(json.dumps(summary, indent=2))

    sub = ROOT / "submissions/task2_fusion_rank_gate.npy"
