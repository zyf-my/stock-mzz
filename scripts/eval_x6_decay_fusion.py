"""Swap locked x6 GRU for input-decay variant. Fusion weights stay frozen.

Does not overwrite task1_fusion_next6_wt_mlp6.npy.

Usage:
    .\\.venv\\Scripts\\python.exe -u scripts\\eval_x6_decay_fusion.py
"""
...
def main() -> None:
    ...
    x6_v = np.load(ROOT / "outputs/gru_x6_input_decay_valid.npy")
    x6_t = np.load(ROOT / "outputs/gru_x6_input_decay_test.npy")
    ...
    print(f"x6_decay RankIC={mean_rank_ic(x6_v, y, my):.6f}")
    ...
    out_dir = ROOT / "outputs" / "x6_decay_fusion"
    out_dir.mkdir(parents=True, exist_ok=True)
    np.save(out_dir / "valid.npy", out_v)
    np.save(out_dir / "test.npy", out_t)
    summary = {
        "x6_decay_ic": float(mean_rank_ic(x6_v, y, my)),
        "x6_locked_ic": float(mean_rank_ic(old_x6_v, y, my)),
    ...
    if summary["passes_bar"]:
        save_submission(out_t, ROOT / "submissions/task1_fusion_x6_decay.npy")
        print("saved submissions/task1_fusion_x6_decay.npy")
    else:
