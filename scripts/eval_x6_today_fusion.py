"""Current main fusion: x6 GRU with include_current_day; all other weights frozen.
...
Platform test 0.125679 (prev main 0.125588). Does not overwrite task1_fusion_next6_wt_mlp6.npy.

Usage:
    .\\.venv\\Scripts\\python.exe -u scripts\\eval_x6_today_fusion.py
"""
...
def main() -> None:
    ...
    print(f"fusion RankIC={ic:.6f} (prev valid={PREV_VALID:.6f})")

    out_dir = ROOT / "outputs" / "x6_today_fusion"
    out_dir.mkdir(parents=True, exist_ok=True)
    np.save(out_dir / "valid.npy", out_v)
    np.save(out_dir / "test.npy", out_t)
    np.save(ROOT / "outputs/fusion_x6_today_valid.npy", out_v)
    np.save(ROOT / "outputs/fusion_x6_today_test.npy", out_t)
    summary = {
        "x6_today_ic": float(mean_rank_ic(x6_v, y, my)),
        "x6_no_today_ic": float(mean_rank_ic(old_x6_v, y, my)),
        ...
        "delta_valid_vs_prev": float(ic - PREV_VALID),
        "delta_test_vs_prev": float(PLATFORM_TEST - PREV_TEST),
        "recipe": "x6 include_current_day; gate2; next6/only6/mlp weights unchanged",
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(json.dumps(summary, indent=2))

    sub = ROOT / "submissions/task1_fusion_x6_today.npy"
    save_submission(out_t, sub)
    (ROOT / "outputs/fusion_x6_today_lock.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(f"wrote {sub}")
