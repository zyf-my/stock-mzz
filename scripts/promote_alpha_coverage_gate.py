"""Promote coverage-conditioned blending of the current candidate and a diverse branch."""
...
def main() -> None:
    ...
    np.save(OUT / "fusion_alpha_coverage_gate_valid.npy", out_v)
    np.save(OUT / "fusion_alpha_coverage_gate_test.npy", out_t)
    save_submission(out_t, ROOT / "submissions/task1_fusion_alpha_coverage_gate.npy")
    (OUT / "fusion_alpha_coverage_gate_lock.json").write_text(
        json.dumps(
            {
                "valid_ic": float(ic),
    ...
    print("WROTE", "submissions/task1_fusion_alpha_coverage_gate.npy")
