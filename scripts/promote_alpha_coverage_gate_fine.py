"""Promote the fine-tuned coverage-conditioned blend."""
...
def main() -> None:
    ...
    np.save(OUT / "fusion_alpha_coverage_gate_fine_valid.npy", out_v)
    np.save(OUT / "fusion_alpha_coverage_gate_fine_test.npy", out_t)
    save_submission(out_t, ROOT / "submissions/task1_fusion_alpha_coverage_gate_fine.npy")
    (OUT / "fusion_alpha_coverage_gate_fine_lock.json").write_text(
        json.dumps(
            {
                "valid_ic": float(ic),
    ...
    print("WROTE", "submissions/task1_fusion_alpha_coverage_gate_fine.npy")
