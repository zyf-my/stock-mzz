"""Test a small original alpha-disagree branch on the current platform winner."""
...
def main() -> None:
    ...
    b_v = np.load(OUT / "fusion_alpha_disagree_valid.npy")
    b_t = np.load(OUT / "fusion_alpha_disagree_test.npy")
    ...
    rows = []
    for wd in np.arange(0.0, .301, .025):
        out = (wd * br_v + (1.0 - wd) * ar_v).astype(np.float32)
        s = rank_ic_series(out, valid["y1"], valid["mask_y"])
        rows.append((float(mean_rank_ic(out, valid["y1"], valid["mask_y"])), float(np.nanmean(s[-60:])), float(wd)))
    rows.sort(reverse=True)
    print("BEST", rows[:30])
    (OUT / "platform_mix_disagree_grid.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
