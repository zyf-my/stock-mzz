"""Search conservative interpolations around the two strongest platform-tested candidates."""
...
def main() -> None:
    ...
    rows = []
    for wb in np.arange(0.0, 1.001, .025):
        out = (wb * br_v + (1.0 - wb) * ar_v).astype(np.float32)
        s = rank_ic_series(out, valid["y1"], valid["mask_y"])
        rows.append((float(mean_rank_ic(out, valid["y1"], valid["mask_y"])), float(np.nanmean(s[-60:])), float(wb)))
    rows.sort(reverse=True)
    print("BEST", rows[:30])
    (OUT / "platform_winner_interpolation_grid.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
