...
from src.dataset import industry_panel, load_eval_splits, resolve_label_key, split_label_array  # noqa: E402
...
def main() -> None:
    ...
    valid = splits["valid"]
    test = splits["test"]
    print(f"loaded {src} label_key={label_key} in {time.perf_counter() - t0:.1f}s")
    ...
    locked = model.fit(
        temporal_valid,
        cs_valid,
        split_label_array(valid, label_key),
        valid["mask_y"],
        valid["mask_x"],
    ...
    confirm = mean_rank_ic(valid_pred, split_label_array(valid, label_key), valid["mask_y"])
